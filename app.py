...
"""Application factory and command-line entry point for ReelFire_TBB."""
from __future__ import annotations
import argparse
import atexit
import secrets
...
from services.analysis_service import AnalysisService
from services.file_service import FileService, FileValidationError
from services.job_service import (CorruptDataError, InvalidJobIdError, JobNotFoundError, JobService, JobStateConflictError)
def create_app(test_config: dict[str, Any] | None = None) -> Flask:
    app = Flask(__name__)
    app.config.from_object(Config)
    if test_config:
    ...
    jobs = JobService(outputs_dir)
    files = FileService(app.config["ALLOWED_VIDEO_EXTENSIONS"])
    analysis = AnalysisService(jobs, app.config["BACKGROUND_WORKERS"], Path(app.config["MODEL_PATH"]))
    app.extensions["job_service"] = jobs
    app.extensions["file_service"] = files
    app.extensions["analysis_service"] = analysis
    jobs.recover_interrupted_jobs()
    ...
    return app
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ReelFire 智能视频分析后端")
    parser.add_argument("--host", default=Config.HOST, help="监听地址")
    parser.add_argument("--port", default=Config.PORT, type=int, help="监听端口")
    parser.add_argument("--debug", action="store_true", help="启用 Flask 调试模式")
    return parser.parse_args()
if __name__ == "__main__":
    arguments = parse_args()
    create_app().run(host=arguments.host, port=arguments.port, debug=arguments.debug)

...
带 SQLite 索引的项目任务已在详情、删除、分析、审核文件写回、粗剪、
报告、editor API、editor 页面和输出文件入口统一校验
`projects.owner_id`。匿名访问返回 `AUTH_REQUIRED`，其他用户访问返回
`JOB_ACCESS_DENIED`。没有 SQLite 索引的旧文件任务继续保持兼容访问。

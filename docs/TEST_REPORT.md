...
| 浏览器控制台错误 | 0 |
| 浏览器失败请求 | 0 |
| 浏览器存储型 XSS 回归 | 2/2 通过 |
| 有音频 16:9 粗剪 | 通过 |
| 无音频 9:16 粗剪 | 通过，1080×1920 |
| 真实 Bug | 3 项，均已回归 |

完整浏览器流程的成功任务为 `20260718_182012_7cc6e7f2`，损坏视频失败任务为 `20260718_182104_7cd30970`。运行产物保存在本机 `outputs/`，截图保存在 `docs/Acceptance_screenshot/`。
...
| N-03 | 无音频真实分析 | 上传 `gameplay_no_audio.mp4`，2 秒采样，执行 YOLO | 任务 `20260718_181845_80766b6d` 完成；采样 27 帧，关键帧 8 张，片段标签 3 类 | PASS |
...
| E-05 | 含 `ftyp` 但无法解码 | 创建后分析进入 `failed`，保留错误 | 任务 `20260718_181857_4be755ac`：`无法读取视频信息，文件可能已损坏或不受支持` | PASS |
...
结果分别为 37/37、10/10、无损坏依赖、JavaScript 语法通过。浏览器回归还会打开 JSON 报告，校验 `segment_tags` 与 `ai_cover_prompt` 存在，并验证项目名和持久化备注两类 XSS 载荷无法执行。

## 7. Bug 回归与限制

三个真实 Bug 的现象、原因、修复和回归见 `BUG_RECORD.md`：伪装格式上传、前后端字段契约不一致和存储型 XSS 均已修复。

本报告对应的 2026-07-18 基线采用通用 `yolo11n.pt`，尚未自训练。它能输出真实 COCO 类别、置信度和边界框，但不宣称识别击杀、残局等 FPS 专用事件；精彩分数还结合场景变化和运动强度，以降低单一通用类别的局限。现有 `segment_tags` 与 `ai_cover_prompt` 仅为规则输出，独立 Agent、知识库、三工具链和调用日志须在四天课程补齐后另行测试。

...
- 2026-07-18 基线使用通用 `yolo11n.pt`，尚未自训练；其 FPS 专用事件识别能力有限，报告只展示真实 COCO 类别；
- 基线中的 `segment_tags` 与 `ai_cover_prompt` 是确定性规则，不是独立 Agent；知识库、三工具调用链和调用日志列入四天补齐计划；
...
6. 后续功能分支只有在重新通过自动化、本机和浏览器验收后才允许合入 `main`。

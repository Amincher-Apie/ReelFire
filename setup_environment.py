...
def install_application_dependencies(*, dry_run: bool) -> None:
    ...
    run(
        # Do not blindly upgrade setuptools here. Some PyTorch releases impose
        # an upper bound (for example, torch 2.11 requires setuptools<82).
        # The selected torch wheel already installs a compatible version.
        pip_command("install", "--upgrade", "pip", "wheel"),
        dry_run=dry_run,
    )
    run(
